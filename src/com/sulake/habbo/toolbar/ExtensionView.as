package com.sulake.habbo.toolbar
{
   ...
   public class ExtensionView implements IExtensionView
   {
      ...
      public function ExtensionView(param1:IHabboWindowManager, param2:IAssetLibrary, param3:HabboToolbar)
      {
         ...
         if(_loc4_)
         {
            this.var_140 = param1.buildFromXML(_loc4_.content as XML,1) as IItemListWindow;
         }
         var _loc5_:IWindowContainer = this.var_140.desktop;
         if(this.var_140)
         {
            this.var_140.x = _loc5_.width - this.var_140.width - const_1123;
            this.var_140.y = const_1123;
            this.var_140.visible = true;
         }
         else
         {
            Core.error("Unable to initialize Toolbar Extension view window from xml asset",false,Core.const_1392);
         }
         this._items = new Map();
      }
      ...
      public function get screenHeight() : uint
      {
         ...
         return this.var_140.height + this.var_140.y;
      }
      ...
      public function attachExtension(param1:String, param2:IWindow, param3:int = -1) : void
      {
         ...
         if(this.var_140)
         {
            if(param3 == -1)
            {
               this.var_140.addListItem(param2);
            }
            else
            {
               this.var_140.addListItemAt(param2,param3);
            }
            this.var_140.invalidate();
         }
      ...
      public function detachExtension(param1:String) : void
      {
         ...
         if(this.var_140)
         {
            this.var_140.removeListItem(_loc2_);
         }
      ...
      private function onResizeTimer(param1:TimerEvent) : void
      {
         if(this._toolbar)
         {
            this._toolbar.events.dispatchEvent(new ExtensionViewEvent(ExtensionViewEvent.const_1361));
         }
      }
   }
}
