package com.sulake.habbo.toolbar.extensions.purse
{
   ...
   public class CurrencyIndicatorBase implements ICurrencyIndicator
   {
      ...
      private var var_742:IBitmapWrapperWindow;
      
      private var var_3021:uint;
      ...
      private var var_313:Vector.<BitmapData>;
      
      private var var_3019:uint;
      
      private var var_3018:String = null;
      
      private var var_2194:uint = 0;
      ...
      {
         this.var_313 = new Vector.<BitmapData>();
         super();
         this._view = null;
      ...
      protected function createWindow(param1:String, param2:String) : void
      {
         ...
         if(_loc3_)
         {
            ...
            if(this._view)
            {
               ...
               {
                  this.var_742 = _loc4_[0] as IBitmapWrapperWindow;
      ...
      protected function animateIcon(param1:int) : void
      {
         ...
         if(this.var_742 && this.var_313.length > 0)
         {
            if(this.var_2194 == ANIM_DIRECTION_FORWARD)
            {
               this.var_312 = 0;
            }
            else
            {
               this.var_312 = this.var_313.length - 1;
            }
            this.var_741 = new Timer(this.var_3019,this.var_313.length);
      ...
      protected function setText(param1:String) : void
      {
         if(this._view)
         {
            this._view.findChildByName(this.var_2192).caption = param1;
            this._view.findChildByName(this.var_2193).caption = param1;
         }
      }
      ...
      protected function setTextUnderline(param1:Boolean) : void
      {
         ...
         if(this._view)
         {
            _loc2_ = this._view.findChildByName(this.var_2192) as ITextWindow;
            _loc3_ = this._view.findChildByName(this.var_2193) as ITextWindow;
      ...
      protected function set iconAnimationSequence(param1:Array) : void
      {
         ...
         for each(_loc2_ in param1)
         {
            ...
            if(_loc3_)
            {
               ...
               if(_loc4_)
               {
                  this.var_313.push(_loc4_.clone());
               }
            }
         }
      }
      ...
      private function onAnimationTimer(param1:TimerEvent) : void
      {
         if(this.var_742 && this.var_313.length > 0)
         {
            this.setIconBitmap(this.var_313[this.var_312]);
            if(this.var_2194 == ANIM_DIRECTION_FORWARD)
            {
               ++this.var_312;
               this.var_312 = this.var_312 < this.var_313.length ? int(this.var_312) : int(this.var_313.length - 1);
            }
            else
            {
               --this.var_312;
               this.var_312 = this.var_312 >= 0 ? int(this.var_312) : 0;
            }
         }
      }
      ...
      private function onAnimationTimerComplete(param1:TimerEvent) : void
      {
         ...
         {
            this.setIconBitmap(this.var_313[0]);
         }
      }
      ...
      private function setIconBitmap(param1:BitmapData) : void
      {
         if(this.var_742)
         {
            this.var_742.bitmap = param1.clone();
            this.var_742.width = param1.width;
            this.var_742.height = param1.height;
            this.var_742.invalidate();
         }
      }
      
      private function onContainerMouseOver(param1:WindowMouseEvent) : void
      {
         this._view.color = this.var_3020;
      }
      
      private function onContainerMouseOut(param1:WindowMouseEvent) : void
      {
         this._view.color = this.var_3021;
      }
   }
}
