package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_409 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_710 && this.var_1630)
         {
            _loc5_ = this.var_26.manager.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_117,this.var_1087,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_41,parseInt(FigureData.const_795));
         ...
         if(this.var_409)
         {
            if(this.var_409.bitmap)
            {
               this.var_409.bitmap.dispose();
            }
            this.var_409.bitmap = new BitmapData(this.var_409.width,this.var_409.height,true,0);
            _loc7_ = (this.var_409.width - _loc1_.width) / 2;
            _loc8_ = (this.var_409.height - _loc1_.height) / 2;
            this.var_409.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_710 = this.var_26.figureData.getFigureString();
               this.var_1087 = this.var_26.gender;
               this.var_26.handler.saveWardrobeOutfit(this.var_2090,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_710)
               {
                  this.var_26.loadAvatarInEditor(this.var_710,this.var_1087,this.var_26.clubMemberLevel);
               }
         }
      }
