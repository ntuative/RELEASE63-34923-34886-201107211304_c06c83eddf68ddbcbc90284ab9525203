package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      private var var_189:AvatarRenderManager;
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_484.getValue(_loc4_) == null)
               {
                  this.var_484.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         if(param1 == null || this.var_586 == null)
         {
            return;
         }
      ...
      private function notifyListeners() : void
      {
         ...
         {
            _loc1_ = this.var_586.pop() as IPetDataListener;
      ...
      {
         var _loc3_:IPetData = this.var_484.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      public function createPetFigureString(param1:int, param2:int, param3:uint) : String
      {
         ...
         var _loc4_:PetData = this.var_484.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_484.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_484.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.RoomSessionChatEvent.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1632.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_711.getValue(_loc6_) == null)
               {
                  ...
                  this.var_711.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_586.indexOf(param3) == -1)
                  {
                     this.var_586.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_711.getKeys())
         {
            _loc5_ = this.var_711.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
