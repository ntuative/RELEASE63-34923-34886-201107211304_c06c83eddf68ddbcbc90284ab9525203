package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private static var var_1537:String = "%";
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. Failed to load URL " + this.getKey("external.variables.txt") + "HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1287);
      }
      ...
      private function onInitConfiguration(param1:Event = null) : void
      {
         ...
         if(_loc3_ == null || _loc3_.length == 0)
         {
            ...
            Core.error("Could not load external variables, got empty data from URL " + this.getKey("external.variables.txt") + " data length = " + _loc5_ + ". Client startup failed!",false,Core.const_1287);
         }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         for each(line in rows)
         {
            if(line.substr(0,1) != "#")
            {
               ...
               if(pair[0].length > 0 && pair[1].length > 0)
               {
                  ...
                  if(key == "configuration.readonly" && value == "SetClothingChangeDataMessageComposer")
                  {
                     readOnly = true;
                  }
         ...
         if(this.getKey("local.environment") == "SetClothingChangeDataMessageComposer")
         {
            for(localKey in this.var_834)
            {
      ...
      {
         var _loc2_:String = this.var_834[param1] as String;
         return _loc2_ != null;
      }
      
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         var _loc4_:String = this.var_834[param1] as String;
         if(_loc4_ == null)
         {
            _loc4_ = param2;
         }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1537);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1537,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
      ...
      {
         var _loc3_:String = this.var_834[param1] as String;
         if(_loc3_ == null)
         {
            return param2;
         }
         return _loc3_ == "1" || _loc3_.toLowerCase() == "SetClothingChangeDataMessageComposer";
      }
   }
}
