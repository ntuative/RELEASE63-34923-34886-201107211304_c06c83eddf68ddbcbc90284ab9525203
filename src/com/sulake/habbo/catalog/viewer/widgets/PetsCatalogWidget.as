package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_537 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_537 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2424 = _loc3_.localizationId;
         this.var_427 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_537,this);
               if(_loc5_)
               {
         ...
         this._offers.add(this.var_537,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_969,this.onApproveNameResult);
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_48)
         {
            ...
            if(this.var_237 == null || _loc3_ >= this.var_237.length)
            {
               return;
            }
            this.var_926 = _loc3_;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_603 = param1.index;
         if(this.var_603 < 0 || this.var_603 > this.var_427.length)
         {
            this.var_603 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2424)
         {
            this.var_237 = this.parseSellableBreeds(param1.sellableBreeds);
            this.var_926 = 0;
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_537 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         {
            _loc5_ = this.var_237[_loc3_] as int;
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_537,_loc5_),this.getRaceLocalizationKey(this.var_537,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_237 == null || this.var_926 >= this.var_237.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_603 >= 0 && this.var_603 < this.var_427.length)
         {
            _loc7_ = this.var_427[this.var_603] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_237[this.var_926] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_537,_loc3_,_loc2_,AvatarScaleType.const_61,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_384,AvatarAction.const_822);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_603 >= this.var_427.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_427[this.var_603] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_237[this.var_926] as int;
      ...
      private function updateAvailableBreeds(param1:String) : void
      {
         ...
         if(this.var_237 == null)
         {
            ...
            this.var_237 = this.parseSellableBreeds(_loc2_);
            this.var_926 = 0;
            if(this.var_237 != null)
            {
               this.updateBreedSelections();
