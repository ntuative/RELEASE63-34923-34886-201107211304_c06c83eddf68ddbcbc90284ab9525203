package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1705:int = 320;
      
      private static const const_1707:int = 48;
      
      private static const const_1706:int = 2;
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_420)
         {
            for each(_loc1_ in this.var_420)
            {
               _loc1_.dispose();
            }
      ...
      public function open() : void
      {
         ...
         if(!this._view)
         {
            ...
            this._view.x = _loc2_.right + const_1706;
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_420.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_420.add(_loc3_.type,_loc1_);
               this.var_63.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_420.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_420.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_63.removeListItem(_loc1_.window);
               this.var_420.remove(this.var_420.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_63.scrollableRegion.height;
         this.var_63.height = Math.max(Math.min(_loc5_,const_1705),const_1707);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1362,true);
      }
   }
}
